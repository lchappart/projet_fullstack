...
): bool | string
{
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $query="INSERT INTO sales_points (manager, siret_siren, address, opening_hours) VALUES (:manager, :siret_siren, :address, :opening_hours)";
    $prep = $pdo->prepare($query);
...
): bool | string
{
    ...
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $query="UPDATE sales_points SET manager = :manager, siret_siren = :siret_siren, address = :address, opening_hours = :opening_hours WHERE id = :id";
    $prep = $pdo->prepare($query);
    $prep->bindValue(':manager', $manager);
    $prep->bindValue(':siret_siren', $siretSiren);
    $prep->bindValue(':address', $address);
